...
void LCD_LayerInit(void)
{
  ...
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((LCD_PIXEL_WIDTH * LCD_BYTES_IN_PIXEL) + 3);

  /* the pitch is the increment from the start of one line of pixels to the 
  start of the next line in bytes, then :
  Pitch = Active high width x number of bytes per pixel */ 
  LTDC_Layer_InitStruct.LTDC_CFBPitch = (LCD_PIXEL_WIDTH * LCD_BYTES_IN_PIXEL);
...
void LCD_Clear(uint16_t Color)
{
  ...
  {
    *(__IO uint16_t*)(CurrentFrameBuffer + index) = Color;
  } 
}
...
void LCD_DrawFullRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  Xaddress = CurrentFrameBuffer + LCD_BYTES_IN_PIXEL*(LCD_PIXEL_WIDTH*Ypos + Xpos);
